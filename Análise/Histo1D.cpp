...
void Histo1D(char* nArq, double xmax = 50000, int nDiv = 100, double p2 = 1500){

    TH1F *Histo1D = new TH1F("Energias", "", nDiv, 0, xmax);
    ...
    Histo1D->SetXTitle("#bf{[eV]}");
    Histo1D->SetYTitle("Contagem");
...
int main(int argc,char** argv){
    ...
    switch (argc){
        ...
        case 5:
            ...
            divisao = argv[3];
            parametro = argv[4];
            Histo1D(argv[1], std::stod(maximo), std::stoi(divisao), std::stod(parametro));
            break;
        default:
            break;
    }
